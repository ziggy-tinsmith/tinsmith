function feature35() {
  ...
  return feature35_core(parameters);
}

function feature35_core(parameters) {
  if (parameters == null) {
    return null;
  }
  var result = {};
  ...
  A = R - I / 2.0;
  B = r - I / 2.0;
  result["VL"] = I * ((2.0 * Math.PI * A * Math.sin((O / 2.0) * Rad)) * A * O * P + (2.0 * Math.PI * (N + r * Math.sin((O + (W / 2.0)) * Rad))) * r * W * P + 2.0 * Math.PI * (G + I / 2.0) * U) * (1.0 / Math.pow(10.0, 6.0));
  return result;
}
